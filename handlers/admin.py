...
async def admin_panel(message: Message, state: FSMContext, db_path: str):
...
async def back_to_admin_menu(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def bot_settings_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
    await state.clear()
    lang = await crud.get_user_language(callback.from_user.id, db_path=db_path)
...
async def edit_price_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
    await state.set_state(AdminSettings.entering_price)
    lang = await crud.get_user_language(callback.from_user.id, db_path=db_path)
    default_price = _t(lang, 'price_text')
    current_price = await crud.get_setting("price_text", default_price, db_path=db_path)
...
async def process_new_price(message: Message, state: FSMContext, db_path: str):
    await crud.set_setting("price_text", message.text, db_path=db_path)
    await state.clear()
    lang = await crud.get_user_language(message.from_user.id, db_path=db_path)
...
async def edit_portfolio_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def process_new_portfolio(message: Message, state: FSMContext, db_path: str):
    await crud.set_setting("portfolio_link", message.text, db_path=db_path)
    await state.clear()
    lang = await crud.get_user_language(message.from_user.id, db_path=db_path)
...
async def edit_channel_id_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def process_new_channel_id(message: Message, state: FSMContext, db_path: str):
    await crud.set_setting("channel_id", message.text, db_path=db_path)
    await state.clear()
    lang = await crud.get_user_language(message.from_user.id, db_path=db_path)
...
async def edit_channel_link_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def process_new_channel_link(message: Message, state: FSMContext, db_path: str):
    await crud.set_setting("channel_link", message.text, db_path=db_path)
    await state.clear()
    lang = await crud.get_user_language(message.from_user.id, db_path=db_path)
...
async def add_slots_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def view_schedule_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def delete_slots_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def admin_cancel_start_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def process_admin_calendar(callback: CallbackQuery, state: FSMContext, db_path: str):
    ...
    if action == "day":
        ...
        if current_state == AdminSchedule.choosing_date_for_add.state:
            ...
            async with crud.aiosqlite.connect(db_path) as db:
            ...
            today_str = selected_date.strftime("%Y-%m-%d")
            free_slots = await crud.get_free_slots_with_ids(today_str, db_path=db_path)
...
async def process_slots_input(message: Message, state: FSMContext, db_path: str):
    ...
    # Записываем в БД
    await crud.add_schedule_slots(slots_to_add, db_path=db_path)
    await state.clear()
...
async def process_delete_slot(callback: CallbackQuery, db_path: str):
    ...
    await crud.delete_schedule_slot(slot_id, db_path=db_path)
    
    # Обновляем клавиатуру
    free_slots = await crud.get_free_slots_with_ids(date_str, db_path=db_path)
...
async def process_clear_day(callback: CallbackQuery, db_path: str):
    """Очищает все свободные слоты на день."""
    date_str = callback.data.split(":")[1]
    lang = await crud.get_user_language(callback.from_user.id, db_path=db_path)
    
    await crud.delete_all_free_slots(date_str, db_path=db_path)
...
async def manage_admins_handler(callback: CallbackQuery, db_path: str):
    ...
    lang = await crud.get_user_language(callback.from_user.id, db_path=db_path)
    admin_ids = await crud.get_admins(db_path=db_path)
...
async def add_admin_handler(callback: CallbackQuery, state: FSMContext, db_path: str):
...
async def process_add_admin(message: Message, state: FSMContext, db_path: str):
    ...
    await crud.add_admin(new_admin_id, message.from_user.id, db_path=db_path)
    await state.clear()
...
async def remove_admin_handler(callback: CallbackQuery, db_path: str):
...
async def process_remove_admin(callback: CallbackQuery, db_path: str):
    """Удаляет администратора."""
    if callback.from_user.id != settings.ADMIN_ID: return
    admin_id = int(callback.data.split(":")[1])
    lang = await crud.get_user_language(callback.from_user.id, db_path=db_path)
    
    await crud.remove_admin(admin_id, db_path=db_path)
    await callback.answer(_t(lang, 'admin_admin_removed', user_id=admin_id))
    
    # Возврат в меню управления
    await manage_admins_handler(callback, db_path=db_path)

# === ОТМЕНА ЗАПИСЕЙ АДМИНИСТРАТОРОМ ===

@router.callback_query(F.data.startswith("confirm_cancel:"))
async def confirm_cancel_handler(callback: CallbackQuery, db_path: str):
    """Запрашивает подтверждение отмены записи."""
    app_id = int(callback.data.split(":")[1])
    lang = await crud.get_user_language(callback.from_user.id, db_path=db_path)
    
    app_details = await crud.get_appointment_details(app_id, db_path=db_path)
    if not app_details:
...
async def process_admin_cancel_app(callback: CallbackQuery, bot: Bot, db_path: str):
    ...
    # 1. Отменяем в БД
    await crud.cancel_appointment(app_id, db_path=db_path)
    
    # 2. Уведомляем администратора
    await callback.answer(_t(lang, 'admin_app_canceled_success'), show_alert=True)
    
    # 3. Уведомляем пользователя
    user_lang = await crud.get_user_language(user_id, db_path=db_path)
...
async def add_bot_handler(message: Message):
    """Adds a new bot to the master system."""
    if message.from_user.id != settings.ADMIN_ID:
        return

    args = message.text.split()
    if len(args) != 3:
        await message.answer("Usage: /add_bot <token> <owner_id>")
        return
    ...
    try:
        ...
        await message.answer("owner_id must be an integer.")
        return

    try:
        # Validate bot and get ID
        temp_bot = Bot(token=token)
        bot_info = await temp_bot.get_me()
        await temp_bot.session.close()
        
        # Save to master.db
        await master_crud.add_bot(token, owner_id)
        
        # Initialize client DB
        db_path = os.path.join(settings.BOTS_DATA_DIR, f"bot_{bot_info.id}.db")
        await crud.init_db(db_path)
        
        # Add owner as admin in the new DB
        await crud.add_admin(owner_id, message.from_user.id, db_path=db_path)
        
        await message.answer(f"✅ Бот @{bot_info.username} успешно добавлен!\n\nID: {bot_info.id}\nВладелец: {owner_id}\n\nПожалуйста, перезапустите основной скрипт для активации нового бота.")
    except Exception as e:
        await message.answer(f"❌ Ошибка при добавлении бота: {e}")
